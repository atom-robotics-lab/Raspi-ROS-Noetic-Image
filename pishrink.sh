...
Usage: $0 [-adhrspvzZ] imagefile.img [newimagefile.img]
  ...
  -p         Remove logs, apt archives, dhcp leases, ssh hostkeys, user bash history and user git config
...
if [[ $prep == true ]]; then
  info "Syspreping: Removing logs, apt archives, dhcp leases, ssh hostkeys, user bash history and user git config"
  mountdir=$(mktemp -d)
  mount "$loopback" "$mountdir"
